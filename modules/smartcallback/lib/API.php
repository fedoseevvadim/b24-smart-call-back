...
class API {
     ...
     *  Регистрaция приложения (получение токена и секретного ключа для цифровой подписи) https://smartcallback.ru/docimentation-api/app-registaration/
     */
     ...
     * @throws \Exception
     */
    function apiRequest($method, $POST = []) {
     ...
     * @return array
     */
    private function _signRequest($POST = []) {
     ...
     * @return mixed
     */
    private function _curlRequest($method, $POST = []) {
    ...
    public function getDomains($POST = []) {
    ...
    public function getQueryList($POST = []) {
    ...
    public function callStart($POST = []) {
        $method = 'callStart';
        $POST['token'] = $this->_CLIENT_TOKEN;
        RETURN $this->apiRequest($method, $POST);
    }
